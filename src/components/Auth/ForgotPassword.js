...
// Đã sửa: Import các hàm cụ thể dưới dạng named export
...
function ForgotPassword() {
  ...
  const handleRequestReset = async (e) => {
    ...
    setMessage('');
    try {
      // Sử dụng hàm requestPasswordReset đã import
      const responseMessage = await requestPasswordReset(email); 
      setMessage(responseMessage);
      setStep(2); // Chuyển sang bước nhập OTP
    } catch (err) {
      setError(err.message || 'Yêu cầu đặt lại mật khẩu thất bại. Vui lòng thử lại.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      // Sử dụng hàm resetPassword đã import
      const responseMessage = await resetPassword(email, otpCode, newPassword, confirmNewPassword);
      setMessage(responseMessage + " Bạn có thể đăng nhập ngay bây giờ.");
      // Có thể chuyển hướng về trang đăng nhập sau khi reset thành công
      // navigate('/login'); 
    } catch (err) {
      setError(err.message || 'Đặt lại mật khẩu thất bại. Vui lòng kiểm tra lại OTP hoặc thử lại.');
    } finally {
      setLoading(false);
    }
  };
